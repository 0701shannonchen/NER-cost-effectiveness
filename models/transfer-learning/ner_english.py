...
path = 'finetuned-ner-english/19epoch/' #modify for each configuration
...
def main():
    ...
    tagger = SequenceTagger(hidden_size=256,
                            ...
                            tag_type=tag_type)
    pretrained = SequenceTagger.load("flair/ner-english")
    state_dict = tagger.state_dict()
    unmatched_layers = []
    for a in state_dict.keys():
        if ("linear" in a) or ("crf" in a): 
    ...
    # transfer the pretrained weight
...
def wrapper_train(my_trainer):
    ...
    my_trainer.train(path,
                train_with_dev=True,
                max_epochs=19, #change for different configuration
                learning_rate=0.1,
                mini_batch_size=64,
