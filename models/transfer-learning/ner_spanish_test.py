...
path = 'finetuned-ner-spanish-test/100epoch/' #modify for each configuration
...
def main():
    ...
    # load the model trained by us
    pretrained = SequenceTagger.load("finetuned-ner-spanish/100epoch/final-model.pt")
    ...
    # use wrapper so that pyjouls can measure energy consumption
    wrapper_train(trainer)
    # save time and energy consumption info into a csv
...
def wrapper_train(my_trainer):
    ...
    my_trainer.train(path,
                train_with_dev=True,
                max_epochs=0, # no training, do inference only
                learning_rate=0.1,
                mini_batch_size=64,
